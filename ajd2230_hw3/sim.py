...
class PyBulletSim:
    ...
    def __init__(self, use_random_objects=False, object_shapes=None, gui=True):
        ...
        # p.createConstraint(self._tote1_body_id, -1, -1, -1, jointType=p.JOINT_FIXED, jointAxis=[0, 0, 0], parentFramePosition=[
        #                    0, 0, 0], childFramePosition=self._tote1_position, childFrameOrientation=p.getQuaternionFromEuler([0, 0, 0]))
        ...
        # p.createConstraint(self._tote2_body_id, -1, -1, -1, jointType=p.JOINT_FIXED, jointAxis=[0, 0, 0], parentFramePosition=[
        #                    0, 0, 0], childFramePosition=self._tote2_position, childFrameOrientation=p.getQuaternionFromEuler([0, 0, 0]))
    ...
    def move_tool(self, position, orientation, speed=0.03):
        ...
        target_joint_state = p.calculateInverseKinematics(self.robot_body_id, self.robot_end_effector_link_index, position, targetOrientation = orientation, residualThreshold=1e-5, maxNumIterations=100)
    ...
    def get_grasp_position_angle(self, object_id):
        ...
        grasp_angle = p.getEulerFromQuaternion(quat)

        # ==================================
        return position, grasp_angle[0]#+(np.pi/2)
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        self.move_tool(pre_grasp_position_over_bin, gripper_orientation, 0.01)
        
        # 3. Move gripper to pre_grasp_position_over_object
        
        self.move_tool(pre_grasp_position_over_object, gripper_orientation, 0.01)
        
        # 4. Move gripper to grasp_position
        
        self.move_tool(grasp_position, gripper_orientation, 0.01)
        ...
        self.move_tool(post_grasp_position, gripper_orientation, 0.01)

        # 7. Move robot to robot_home_configuration
        
        self.robot_go_home(0.01)
        #self.ste
    ...
    def check_collision(self, q):
        ...
        for obstacle_id in self.obstacles:
            closest_points = p.getClosestPoints(
                self.robot_body_id, obstacle_id, 0.18)
